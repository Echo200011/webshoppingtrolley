...
public interface SpuRepository extends JpaRepository<Spu, Integer> {
  ...
  @Query(value = "select t1.*,t2.name,t4.name from spu as t1 "
      ...
      + "and if(:#{#parameter.maxPrice}!='',t1.price between :#{#parameter.minPrice} and :#{#parameter.maxPrice} ,1=1)", nativeQuery = true)
  Page<Spu> findAll(@Param("parameter") SpuQueryParameter parameter, Pageable pageable);
